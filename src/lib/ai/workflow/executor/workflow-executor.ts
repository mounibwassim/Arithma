...
import { createGraphStore, type WorkflowRuntimeState } from "./graph-store";
import { createStateGraph, graphNode, type StateGraphRegistry } from "ts-edge";
import {
...
import type { DBEdge, DBNode } from "app-types/workflow";
...
export const createWorkflowExecutor = (workflow: {
  ...
  const inputNode = workflow.nodes.find((node) => node.kind === NodeKind.Input);
  if (!inputNode) {
