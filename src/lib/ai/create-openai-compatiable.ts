...
export function createOpenAICompatibleModels(
	...
	const visionUnsupportedModels = new Set<LanguageModel>();

	if (!config?.length) {
		return { providers, unsupportedModels, visionUnsupportedModels };
	}
	try {
			...
			if (provider === "Azure OpenAI") {
				...
				models.forEach(
					({
						...
						if (!supportsVision) {
							visionUnsupportedModels.add(model);
						}
					},
				);
			} else {
				// Standard OpenAI-compatible providers (original implementation)
				models.forEach(({ apiName, uiName, supportsTools, supportsVision }) => {
...
const OpenAICompatibleModelSchema = z.object({
	...
	supportsVision: z
		...
		.default(false)
		.describe(
			"Indicates if the model supports multi-modal vision input (images).",
		),
	apiVersion: z
		.string()
		.optional()
