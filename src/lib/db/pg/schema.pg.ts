...
import type { DBWorkflow, DBEdge, DBNode } from "app-types/workflow";
...
// Separate Admin Credentials Table (not linked to User table)
...
// Academic Resources Table for storing lecture notes and tutorials
export const AcademicResourceTable = pgTable("academic_resource", {
  id: uuid("id").primaryKey().notNull().defaultRandom(),
  title: text("title").notNull(),
  description: text("description"),
  fileName: text("file_name").notNull(),
  fileType: varchar("file_type", { enum: ["pdf", "word"] }).notNull(),
  fileSize: text("file_size").notNull(), // stored as string for large file sizes
  fileData: text("file_data").notNull(), // base64 encoded file content
  uploadedBy: uuid("uploaded_by")
    .notNull()
    .references(() => AdminCredentialsTable.id, { onDelete: "cascade" }),
  createdAt: timestamp("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  updatedAt: timestamp("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
});

export type AcademicResourceEntity = typeof AcademicResourceTable.$inferSelect;

// Survey Response Table for storing user feedback
export const SurveyResponseTable = pgTable("survey_response", {
  id: uuid("id").primaryKey().notNull().defaultRandom(),
  userId: uuid("user_id").references(() => UserTable.id, {
    onDelete: "cascade",
  }), // null for guests
  userEmail: text("user_email"), // for guest identification
  userName: text("user_name"), // name of user at time of submission
  rating: varchar("rating", { enum: ["like", "dislike"] }).notNull(),
  feedback: text("feedback"),
  createdAt: timestamp("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
});

export type SurveyResponseEntity = typeof SurveyResponseTable.$inferSelect;
