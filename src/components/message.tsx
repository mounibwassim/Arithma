...
const PurePreviewMessage = ({
  ...
  // Find the index of the last text part (for showing action buttons)
  // This ensures buttons show on text parts even if reasoning parts come after
  const lastTextPartIndex = useMemo(() => {
    for (let i = partsForDisplay.length - 1; i >= 0; i--) {
      if (partsForDisplay[i].type === "text") {
        return i;
      }
    }
    return -1;
